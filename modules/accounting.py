...
class Accounting:
    """Double-entry accounting services used by POS transactions."""
    ...
    def ensure_schema(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_journal_date ON journal_entries(created_at); CREATE INDEX IF NOT EXISTS idx_journal_account ON journal_lines(account_id);
        ''')
    ...
    def account_id(self,name): return self.db.conn.execute('SELECT id FROM accounts WHERE name=?',(name,)).fetchone()['id']
    def post(self,reference,description,lines,business_day_id=None,user_id=None):
        ...
        now=datetime.now().isoformat(timespec='seconds'); entry_no='JE-'+datetime.now().strftime('%Y%m%d%H%M%S%f'); cur=self.db.conn.cursor()
        cur.execute('INSERT INTO journal_entries(entry_no,created_at,reference,description,business_day_id,user_id) VALUES(?,?,?,?,?,?)',(entry_no,now,reference,description,business_day_id,user_id)); jid=cur.lastrowid
    ...
    def sale(self,reference,total,cost,payment,customer_id=None,tax=0,discount=0,business_day_id=None,user_id=None):
        debit_account='Cash' if payment=='Cash' else 'Bank / Card'
        lines=[(debit_account,total,0),('Sales Revenue',0,max(0,total-tax)),('Tax Payable',0,max(0,tax))]
        if cost: lines += [('Cost of Goods Sold',cost,0),('Inventory',0,cost)]
