...
class Database:
    def __init__(self, path=DB_PATH):
        ...
        self.conn.execute('PRAGMA foreign_keys=ON'); self.conn.execute('PRAGMA journal_mode=WAL'); self.conn.execute('PRAGMA synchronous=NORMAL'); self.setup()
    def setup(self):
        ...
        CREATE INDEX IF NOT EXISTS idx_orders_created ON orders(created_at); CREATE INDEX IF NOT EXISTS idx_stock_product ON stock_movements(product_id,created_at); CREATE INDEX IF NOT EXISTS idx_customer_tx ON customer_transactions(customer_id,created_at); CREATE INDEX IF NOT EXISTS idx_supplier_tx ON supplier_transactions(supplier_id,created_at); CREATE INDEX IF NOT EXISTS idx_cash_day ON cash_transactions(business_day_id,created_at); CREATE INDEX IF NOT EXISTS idx_audit_created ON audit_logs(created_at);
        ''')
        for u,role in [('admin','Admin'),('owner','Owner')]: self.conn.execute("INSERT OR IGNORE INTO users(username,password,role) VALUES(?,?,?)",(u,'0099',role)); self.conn.execute("UPDATE users SET password='0099',role=?,active=1 WHERE username=?",(role,u))
        ...
        for code,name,typ in [('1000','Cash','Asset'),('1010','Bank / Card','Asset'),('1100','Accounts Receivable','Asset'),('1200','Inventory','Asset'),('2000','Accounts Payable','Liability'),('3000','Owner Equity','Equity'),('4000','Sales Revenue','Revenue'),('4100','Discounts','Contra Revenue'),('5000','Cost of Goods Sold','Expense'),('6000','Operating Expenses','Expense'),('7000','Tax Payable','Liability')]: self.conn.execute('INSERT OR IGNORE INTO accounts(code,name,account_type) VALUES(?,?,?)',(code,name,typ))
    ...
    def customers(self,search=''): return self.conn.execute("SELECT c.*,COALESCE((SELECT SUM(debit-credit) FROM customer_transactions WHERE customer_id=c.id),0)+c.opening_balance balance FROM customers c WHERE c.name LIKE ? OR COALESCE(c.phone,'') LIKE ? ORDER BY c.name",(f'%{search}%',f'%{search}%')).fetchall()
    def save_customer(self,name,phone,email,opening=0): self.conn.execute('INSERT INTO customers(name,phone,email,opening_balance,created_at) VALUES(?,?,?,?,?)',(name,phone or None,email,float(opening),datetime.now().isoformat(timespec='seconds'))); self.conn.commit()
    def suppliers(self,search=''): return self.conn.execute("SELECT s.*,COALESCE((SELECT SUM(debit-credit) FROM supplier_transactions WHERE supplier_id=s.id),0)+s.opening_balance balance FROM suppliers s WHERE s.name LIKE ? OR COALESCE(s.phone,'') LIKE ? ORDER BY s.name",(f'%{search}%',f'%{search}%')).fetchall()
    def save_supplier(self,name,phone,email,opening=0): self.conn.execute('INSERT INTO suppliers(name,phone,email,opening_balance,created_at) VALUES(?,?,?,?,?)',(name,phone or None,email,float(opening),datetime.now().isoformat(timespec='seconds'))); self.conn.commit()
    ...
    def customer_payment(self,cid,amount,user_id=None,method='Cash',note='Customer payment'):
        b=self.current_business_day(); bid=b['id'] if b else self.open_business_day(0); ref='CP-'+datetime.now().strftime('%Y%m%d%H%M%S%f'); now=datetime.now().isoformat(timespec='seconds'); self.conn.execute('INSERT INTO customer_transactions(customer_id,created_at,type,reference,debit,credit,note,business_day_id) VALUES(?,?,?,?,?,?,?,?)',(cid,now,'PAYMENT',ref,0,float(amount),note,bid));
        if method=='Cash': self.cash(amount,'CUSTOMER_PAYMENT',note,user_id,ref,bid)
        self.audit(user_id,'PAYMENT','customer',cid,ref); self.conn.commit(); return ref
    def supplier_payment(self,sid,amount,user_id=None,method='Cash',note='Supplier payment'):
        b=self.current_business_day(); bid=b['id'] if b else self.open_business_day(0); ref='SP-'+datetime.now().strftime('%Y%m%d%H%M%S%f'); now=datetime.now().isoformat(timespec='seconds'); self.conn.execute('INSERT INTO supplier_transactions(supplier_id,created_at,type,reference,debit,credit,note,business_day_id) VALUES(?,?,?,?,?,?,?,?)',(sid,now,'PAYMENT',ref,float(amount),0,note,bid));
        if method=='Cash': self.cash(-float(amount),'SUPPLIER_PAYMENT',note,user_id,ref,bid)
        self.audit(user_id,'PAYMENT','supplier',sid,ref); self.conn.commit(); return ref
    ...
    def add_expense(self,category,description,amount,payment,user_id=None,bid=None):
        bid=bid or (self.current_business_day()['id'] if self.current_business_day() else None); self.conn.execute('INSERT INTO expenses(created_at,category,description,amount,payment_method,business_day_id,user_id) VALUES(?,?,?,?,?,?,?)',(datetime.now().isoformat(timespec='seconds'),category,description,float(amount),payment,bid,user_id)); ref='EXP-'+datetime.now().strftime('%Y%m%d%H%M%S%f');
        if payment=='Cash': self.cash(-float(amount),'EXPENSE',description,user_id,ref,bid)
    ...
    def trial_balance(self,df=None,dt=None):
        where=[]; args=[]
        if df: where.append('date(j.created_at)>=?'); args.append(df)
        if dt: where.append('date(j.created_at)<=?'); args.append(dt)
        w=(' WHERE '+' AND '.join(where)) if where else ''
        return self.conn.execute(f'''SELECT a.code,a.name,a.account_type,COALESCE(SUM(l.debit),0) debit,COALESCE(SUM(l.credit),0) credit FROM accounts a LEFT JOIN journal_lines l ON l.account_id=a.id LEFT JOIN journal_entries j ON j.id=l.journal_id {w} GROUP BY a.id ORDER BY a.code''',args).fetchall()
